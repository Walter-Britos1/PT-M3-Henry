...
async function problemA () {
  ...
  // AsyncAwait version
  async function functionAsync(file) {
    blue(await promisifiedReadFile(file))
  }

  await Promise.all([functionAsync('poem-two/stanza-01.txt'),functionAsync('poem-two/stanza-02.txt')])
  console.log('done');
};
...
async function problemB () {
  ...
  const allPromisisfile = filenames.map(fn => functionAsync(fn));
...
async function problemC () {
  ...
  // AsyncAwait version
  async function functionAsync(filename) {
    blue( await promisifiedReadFile(filename))
  }
  for (let i = 0; i < filenames.length; i++) {
    await  functionAsync(filenames[i])
  }
